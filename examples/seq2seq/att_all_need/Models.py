...
import numpy as np
from att_all_need.Layers import EncoderLayer, DecoderLayer
...
class Encoder(nn.Module):
    ...
    def forward(self, src_seq, src_mask, extra_mask1, extra_mask2, extra_mask3, return_attns=False):
        ...
        for enc_layer in self.layer_stack:
            enc_output, enc_slf_attn, enc_hidden_states= enc_layer(enc_output,
                                    slf_attn_mask=src_mask, extra_mask1=extra_mask1, extra_mask2=extra_mask2, extra_mask3=extra_mask3)
...
class Transformer(nn.Module):
    ...
    def forward(self, src_seq, trg_seq, src_seq_with_oov, oov_zero, extra_mask1, extra_mask2):
        ...
        if self.use_pointer:
            enc_output, enc_slf_attn_list, enc_hidden_states = self.encoder(src_seq, src_mask,
                                    extra_mask1=extra_mask1, extra_mask2=extra_mask2, extra_mask3=extra_mask3, return_attns=self.use_pointer)
            dec_output, dec_slf_attn_list, dec_enc_attn_list, dec_hidden_states, enc_dec_contexts\
                = self.decoder(trg_seq, trg_mask, enc_output, src_mask, return_attns=self.use_pointer)
            ...
            enc_output = self.encoder(src_seq, src_mask, extra_mask1=extra_mask1, extra_mask2=extra_mask2)
        ...
        if self.use_pointer:
            ...
            # attention_score = dec_enc_attn_list[-1][:, 0, :, :].squeeze(1)
